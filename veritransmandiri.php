...
class VeritransMandiri extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->config_keys = array(			
			...
			'VM_KURS',
			'VM_CONVENIENCE_FEE',
			'VM_PAYMENT_SUCCESS_STATUS_MAP',
			'VM_PAYMENT_FAILURE_STATUS_MAP',
			'VM_PAYMENT_CHALLENGE_STATUS_MAP',
			'VM_ENVIRONMENT',
			'VM_SANITIZED',
			'VM_ENABLE_INSTALLMENT',
			// 'VM_ENABLED_BNI_INSTALLMENT',
			'VM_ENABLED_MANDIRI_INSTALLMENT',
			'VM_INSTALLMENTS_BNI',
			'VM_INSTALLMENTS_MANDIRI'
			);

		foreach (array('BNI', 'MANDIRI') as $bank) {
			foreach (array(3, 6, 12) as $months) {
				array_push($this->config_keys, 'VM_INSTALLMENTS_' . $bank . '_' . $months);
			}
		}
		...
		if (isset($config['VM_KURS']))
			$this->veritrans_kurs = $config['VM_KURS'];
		else
			Configuration::set('VM_KURS', 10000);
		
		Configuration::set('VM_API_VERSION', 2);
		Configuration::set('VM_PAYMENT_TYPE','vtweb');

		if (!isset($config['VM_SANITIZED']))
			Configuration::set('VM_SANITIZED', 0);	
		parent::__construct();

		$this->displayName = $this->l('Veritrans Mandiri Installments');
		$this->description = $this->l('Extends Veritrans payment method to accept payment using Mandiri Installments.');
		$this->confirmUninstall = $this->l('Are you sure about uninstalling Veritrans Mandiri Installments?');
	...
	public function install()
	{
		...
		$order_state->module_name = 'veritransmandiri';
		...
		Configuration::updateValue('VM_API_VERSION', 2);
		...
		include_once(_PS_MODULE_DIR_ . '/' . $this->name . '/vtmandiri_install.php');
		$vtmandiri_install = new VeritransMandiriInstall();
		$vtmandiri_install->createTable();
	...
	private function _postValidation()
	{
		...
		{
			if (Tools::getValue('VM_API_VERSION') == 2)
			{
				if (!Tools::getValue('VM_SERVER_KEY'))
					$this->_postErrors[] = $this->l('Server Key is required.');
			} 

			// validate conversion rate existence
			if (!Currency::exists('IDR', null) && !Tools::getValue('VM_KURS'))
			{
	...
	private function _displayVeritransMandiri()
	{
	...
	private function _displayVeritransMandiriOld()
	{
		$this->_html .= '<img src="../modules/veritransmandiri/Veritrans.png" style="float:left; margin-right:15px;"><b>'.$this->l('Extends Veritrans payment method to accept payment using Mandiri Installments.').'</b><br/><br/>
		'.$this->l('Payment Mandiri Installments via veritrans.').'<br /><br /><br />';
	}
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					// 	'name' => 'VM_ENABLED_BNI_INSTALLMENT',						
					// 	'is_bool' => true,
					// 	'values' => array(
					// 		array(
					// 			'id' => 'VM_ENABLED_BNI_INSTALLMENT_on',
					// 			'value' => 1,
					// 			'label' => 'Yes'
					// 			),
					// 		array(
					// 			'id' => 'VM_ENABLED_BNI_INSTALLMENT_off',
					// 			'value' => 0,
					// 			'label' => 'No'
					// 			)
					// 		),
					// 	//'class' => 'VM_ENABLED_BNI_INSTALLMENT'
					// 	),
					// array(
					// 	'type' => 'text',
					// 	'label' => 'BNI Installment terms',
					// 	'name' => 'VM_INSTALLMENTS_BNI',
					...
					// 	'class' => 'VM_INSTALLMENTS_BNI'	
					// 	),
					array(
						'type' => (version_compare(Configuration::get('PS_VERSION_DB'), '1.6') == -1)?'radio':'switch',
						'label' => 'MANDIRI Installment',
						'name' => 'VM_ENABLED_MANDIRI_INSTALLMENT',						
						'is_bool' => true,
						'values' => array(
							array(
								'id' => 'VM_ENABLED_MANDIRI_INSTALLMENT_on',
								'value' => 1,
								'label' => 'Yes'
								),
							array(
								'id' => 'VM_ENABLED_MANDIRI_INSTALLMENT_off',
								'value' => 0,
								'label' => 'No'
								)
							),
						//'class' => 'VM_ENABLED_MANDIRI_INSTALLMENT'
						),
					array(
						'type' => 'text',
						'label' => 'Mandiri Installment terms',
						'name' => 'VM_INSTALLMENTS_MANDIRI',
						'desc' => 'Insert installment terms in months, separate using coma (,) for multiple value. e.g: 3,6,12',
						//'class' => 'v1_vtweb_settings sensitive'\
						'class' => 'VM_INSTALLMENTS_MANDIRI'	
						),
				/*	array(
						'type' => 'checkbox',
						'label' => 'Enable Mandiri Installments?',
						'name' => 'VM_INSTALLMENTS',
						'values' => array(
					...
					array(
						...
						'name' => 'VM_PAYMENT_SUCCESS_STATUS_MAP',
						'required' => true,	
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VM_PAYMENT_FAILURE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VM_PAYMENT_CHALLENGE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
	...
	private function _displayFormOld()
	{
		...
		$this->context->smarty->assign(array(
			...
			'api_version' => htmlentities(Configuration::get('VM_API_VERSION'), ENT_COMPAT, 'UTF-8'),
			'api_versions' => array(1 => 'v1', 2 => 'v2'),
			//'payment_type' => htmlentities(Configuration::get('VM_PAYMENT_TYPE'), ENT_COMPAT, 'UTF-8'),
			//'payment_types' => array('vtweb' => 'VT-Web', 'vtdirect' => 'VT-Direct'),
			// 'client_key' => htmlentities(Configuration::get('VM_CLIENT_KEY'), ENT_COMPAT, 'UTF-8'),
			'server_key' => htmlentities(Configuration::get('VM_SERVER_KEY'), ENT_COMPAT, 'UTF-8'),
			'environments' => array(false => 'Development', true => 'Production'),
			'environment' => htmlentities(Configuration::get('VM_ENVIRONMENT'), ENT_COMPAT, 'UTF-8'),
			// 'enable_3d_secure' => htmlentities(Configuration::get('VM_3D_SECURE'), ENT_COMPAT, 'UTF-8'),
			// 'enable_sanitized' => htmlentities(Configuration::get('VM_SANITIZED'), ENT_COMPAT, 'UTF-8'),
			...
			'payment_success_status_map' => htmlentities(Configuration::get('VM_PAYMENT_SUCCESS_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_challenge_status_map' => htmlentities(Configuration::get('VM_PAYMENT_CHALLENGE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_failure_status_map' => htmlentities(Configuration::get('VM_PAYMENT_FAILURE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'kurs' => htmlentities(Configuration::get('VM_KURS', $this->veritrans_kurs), ENT_COMPAT, 'UTF-8'),
			//'kurs' => htmlentities(Configuration::get('VM_INSTALLMENTS_BNI', ENT_COMPAT, 'UTF-8'),
			'convenience_fee' => htmlentities(Configuration::get('VM_CONVENIENCE_FEE', $this->veritrans_convenience_fee), ENT_COMPAT, 'UTF-8'),
	...
	public function hookDisplayPayment($params)
	{
		...
		if($num_product == 1 && Configuration::get('VM_ENABLE_INSTALLMENT') == 'certain_product'){
	...
	public function execPayment($cart)
	{
		...
		$smarty->assign(array(
			'payment_type' => Configuration::get('VM_PAYMENT_TYPE'),
			'api_version' => Configuration::get('VM_API_VERSION'),
			'error_message' => '',
			'link' => $link,
...
//
	public function getTermInstallment($name_bank){
		$ans = array();
		foreach ($this->config_keys as $key) {
			if ( (strpos($key, 'VM_INSTALLMENTS_' . $name_bank ) !== FALSE) && (Configuration::get($key) == 'on') ){
				
				$term = Configuration::get('VM_INSTALLMENTS_'.$name_bank);
	...
	public function execValidation($cart)
	{
		...
		foreach (Module::getPaymentModules() as $module)
			if ($module['name'] == 'veritransmandiri')
			{
				$authorized = true;
				break;
			}
		...
		$usd = Configuration::get('VM_KURS');
		$cf = Configuration::get('VM_CONVENIENCE_FEE') * 0.01;
		...
		//SETUP
		Veritrans_Config::$serverKey = Configuration::get('VM_SERVER_KEY');
		Veritrans_Config::$isProduction = Configuration::get('VM_ENVIRONMENT') == 'production' ? true : false;
   ...
   //  	if (Configuration::get('VM_3D_SECURE') == 'on' || Configuration::get('VM_3D_SECURE') == 1)
		...
		//error_log('sanitized '.Configuration::get('VM_SANITIZED'));
		...
		if ($cart_currency->iso_code != 'IDR')
		{
			...
			{
			  // use rate
			  $conversion_func = function($input) { return $input * intval(Configuration::get('VM_KURS')); };
			}
		...
		// $isBniInstallment = Configuration::get('VM_ENABLED_BNI_INSTALLMENT') == 1;
		$isMandiriInstallment = Configuration::get('VM_ENABLED_MANDIRI_INSTALLMENT') == 1;
		$isBniInstallment = false;
		$warning_redirect = false;
		$fullPayment = true;

		$installment_type_val = Configuration::get('VM_ENABLE_INSTALLMENT');
		$param_required;
		switch ($installment_type_val) {
			case 'all_product':
				...
				if ($isBniInstallment){					
					...
					$a = Configuration::get('VM_INSTALLMENTS_BNI');
					$term = explode(',',$a);
					$bni_term = $term;
				...
				if ($isMandiriInstallment){
					...
					$a = Configuration::get('VM_INSTALLMENTS_MANDIRI');
					$term = explode(',',$a);
					$mandiri_term = $term;
	...
	//error_log($param_installment,true);
		...
		//MANDIRI bin Filter
		$binarray[] = 'mandiri';
		if (Configuration::get('VM_ENVIRONMENT') == 'development'){
			$binarray[] = '4';
			$binarray[] = '5';
		}
		$params_all['vtweb']['credit_card_bins'] = $binarray;
		//end of MANDIRI bin Filter
		
		// error_log(print_r(json_encode($params_all),true)); // debugan
		
		if (Configuration::get('VM_API_VERSION') == 2 && Configuration::get('VM_PAYMENT_TYPE') != 'vtdirect') //transaksi https://github.com/veritrans/veritrans-php/blob/vtweb-2/examples/v2/VM_web/checkout_process.php line 77
		{						
			try {
	...
	public function execNotification()
	{
		...
		//{
			...
			if ($veritrans_notification->transaction_status == 'capture')				
		    {
		     	...
		     	{
		       		$history->changeIdOrderState(Configuration::get('VM_PAYMENT_SUCCESS_STATUS_MAP'), $order_id_notif);
		       		echo 'Valid success notification accepted.';
		       	}
		       	else if ($veritrans_notification->fraud_status== 'challenge')
		     	{
		       		$history->changeIdOrderState(Configuration::get('VM_PAYMENT_CHALLENGE_STATUS_MAP'), $order_id_notif);
		     	...
		     	$history->changeIdOrderState(Configuration::get('VM_PAYMENT_FAILURE_STATUS_MAP'), $order_id_notif);
		       	echo 'Pending notification accepted.';
		     }
			 else
		     {
