...
class VeritransMandiriValidationModuleFrontController extends ModuleFrontController
{
  ...
  //   $veritrans->version = Configuration::get('VM_API_VERSION');
  //   $veritrans->environment = Configuration::get('VM_ENVIRONMENT');
  //   $veritrans->payment_type = Configuration::get('VM_PAYMENT_TYPE') == 'vtdirect' ? Veritrans_Config::VM_DIRECT : Veritrans_Config::VM_WEB;
  //   $veritrans->merchant_id = Configuration::get('VM_MERCHANT_ID');
  //   $veritrans->merchant_hash_key = Configuration::get('VM_MERCHANT_HASH');
  //   $veritrans->client_key = Configuration::get('VM_CLIENT_KEY');
  //   $veritrans->server_key = Configuration::get('VM_SERVER_KEY');
  //   $veritrans->enable_3d_secure = Configuration::get('VM_3D_SECURE');
  ...
  //   $this->module->validateOrder($cart->id, Configuration::get('VM_ORDER_STATE_ID'), $cart->getOrderTotal(true, Cart::BOTH), $this->module->displayName, NULL, $mailVars, (int)$currency->id, false, $customer->secure_key);
  ...
  if ($veritrans_api_version == 2 && $veritrans_payment_method == 'vtweb')
  {
      if ($keys['isWarning']){          
          ...
          Tools::redirectLink('index.php?fc=module&module=veritransmandiri&controller=warning&redirlink='.$keys['redirect_url'].'&message='.$keys['message']);
      }      
